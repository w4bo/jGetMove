...
class OptimizedItemsetsFinderTest {
    ...
    static void init() throws IOException, MalformedTimeIndexException, ClusterNotExistException {
        Debug.disable();
        simpleDataBase = new DataBase(new Input("src/test/assets/simple.dat"), new Input("src/test/assets/simple_time_index.dat"));
        config = new Config(1, 0, 1, 0, 0);
        complexDataBase = new DataBase(new Input("src/test/assets/basic.dat"), new Input("src/test/assets/basic_time_index.dat"));
    }
